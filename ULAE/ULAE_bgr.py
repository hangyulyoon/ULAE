...
class SpatialTransformer(nn.Module):
    def __init__(self, size, mode="bilinear"):
        ...
        grids = torch.meshgrid(vectors) # 1d vector들을 넣으면 'vectors'에 저장된 size의 tensor로 반환 -> 이 때 1d vector의 개수와 vectors의 개수는 동일해야 함 
        grid = torch.stack(grids) # dim = 0을 따라 stack
...
class Uncoupled_Encoding_Block(nn.Module):
    ...
    def forward(self, x, y):
        size = [int(i / 2) for i in x.shape[2:]] #128 128 128
        fea_e0 = self.mean(x) #nn.AdaptiveAvgPool3d((1, 1, 1)) torch.Size([1, 1, 1, 1, 1])
        fea_e0 = self.conv(fea_e0) # torch.Size([1, 16, 1, 1, 1])
        fea_e0 = F.upsample(fea_e0, size=size, mode='trilinear') # torch.Size([1, 16, 128, 128, 128])

        e0 = self.conv2(x)
        e1 = self.ab1(x) # nn.Conv3d(in_ch, out_ch, 1, stride=stride)
        e2 = self.ab6(x) # nn.Conv3d(in_ch, out_ch, 3, padding=dilation, dilation=dilation, stride=stride)
        e3 = self.ab12(x) # nn.Conv3d(in_ch, out_ch, 3, padding=dilation * 2, dilation=dilation * 2, stride=stride)
        e4 = self.ab18(x) # nn.Conv3d(in_ch, out_ch, 3, padding=dilation * 3, dilation=dilation * 3, stride=stride)
...
class Accumulative_Enhancement_1(nn.Module):
    ...
    def forward(self, x, y): #torch.Size([1, 1, 256, 256, 256]) , torch.Size([1, 1, 256, 256, 256])

        x_in = torch.cat((x, y), 1) # torch.Size([1, 2, 256, 256, 256])

        down_x_in = F.avg_pool3d(x_in, kernel_size=2, stride=2) # torch.Size([1, 2, 128, 128, 128])
        down_down_x_in = F.avg_pool3d(down_x_in, kernel_size=2, stride=2) # torch.Size([1, 2, 64, 64, 64])


        fe1 = self.fec1(y) # torch.Size([1, 16, 128, 128, 128])
        fe2 = self.fec2(fe1) # torch.Size([1, 32, 64, 64, 64])
        fe3 = self.fec3(fe2) # torch.Size([1, 32, 32, 32, 32])
        fe4 = self.fec4(fe3) # torch.Size([1, 32, 16, 16, 16])

        me1 = F.leaky_relu(self.mec1(x, fe1), negative_slope=0.2) # torch.Size([1, 16, 128, 128, 128])
        me2 = F.leaky_relu(self.mec2(me1, fe2), negative_slope=0.2) # torch.Size([1, 32, 64, 64, 64])
        me3 = F.leaky_relu(self.mec3(me2, fe3), negative_slope=0.2) # torch.Size([1, 32, 32, 32, 32])
        me4 = F.leaky_relu(self.mec4(me3, fe4), negative_slope=0.2) # torch.Size([1, 32, 16, 16, 16])

        e4 = self.fusion4(torch.cat([me4, fe4], dim=1)) # torch.Size([1, 32, 16, 16, 16])
        e3 = self.fusion3(torch.cat([me3, fe3], dim=1)) # torch.Size([1, 32, 32, 32, 32])
        e2 = self.fusion2(torch.cat([me2, fe2], dim=1))
        e1 = self.fusion1(torch.cat([me1, fe1], dim=1))
        # d = decoding
        d0 = self.dc0(e4) # torch.Size([1, 32, 16, 16, 16])

        d1 = self.dc1(torch.cat((self.up(d0), e3), 1)) # torch.Size([1, 32, 32, 32, 32])
        d2 = self.dc4(d1) # torch.Size([1, 32, 32, 32, 32])
        d3 = self.dc5(d2) # torch.Size([1, 16, 32, 32, 32])
        d6 = self.dc6(torch.cat((self.up(d3), down_down_x_in), 1)) # torch.Size([1, 16, 64, 64, 64])
        
        out_v = self.output(d6) # torch.Size([1, 3, 64, 64, 64])
        out_v = self.resize(out_v) # torch.Size([1, 3, 256, 256, 256])
        out_flow = out_v
...
class ULAE_bgr(nn.Module):
    ...
    def fdown_conv(self, in_ch, out_ch, kernel_size=3, stride=2, padding=1, bias=False, batchnorm=False):
        ...
        return layer
    def __init__(self, in_ch, out_ch, basic_channel=16, is_train=True, imgshape=(256, 256, 256)):
        super(ULAE_bgr, self).__init__()
        ...
        fec1 = self.fdown_conv(1, basic_channel) #stride = 2 
        ...
        fusion1 = self.conv_block(basic_channel * 2, basic_channel) #stride = 1
    ...
    def forward(self, x, y,input_image,input_image_two):
        ...
        flowc = self.transformer(flowb, flow3) + flow3 # flowc 가 최종 flow
        

        input_image_warp = self.transformer(input_image,flowc)

        # color label 
        y_source_new_two_b = self.transformer(input_image_two[...,0], flowc) # 한 채널씩 가져와서 변형시킵니다.
        y_source_new_two_g = self.transformer(input_image_two[...,1], flowc)
        y_source_new_two_r= self.transformer(input_image_two[...,2], flowc)
        
        y_source_new_two_b = y_source_new_two_b.unsqueeze(-1)
        y_source_new_two_g = y_source_new_two_g.unsqueeze(-1)
        y_source_new_two_r = y_source_new_two_r.unsqueeze(-1)
        y_source_new_two_end = torch.cat((y_source_new_two_b,y_source_new_two_g), axis=5) # 변형시킨 이미지들 채널을 다시 합칩니다.
        input_image_warp_two = torch.cat((y_source_new_two_end,y_source_new_two_r), axis=5)
        return [warp1, warp2, warp3],[flowa, flowb, flowc],[flow1, flow2, flow3],input_image_warp,input_image_warp_two #input_image_warp_two_2

if __name__=="__main__":
    model = ULAE_bgr(2, 3, 16, True, [256,256,256]).cuda() # 160,192,160
