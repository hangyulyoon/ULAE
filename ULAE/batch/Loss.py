...
class NCC(torch.nn.Module):
    ...
    def __init__(self, win=5, eps=1e-3): #1e-5
    ...
    def forward(self, I, J):
        ndims = 3
        win_size = self.w_temp # 11 

        # set window size
        if self.win is None:
            self.win = [5] * ndims
        else:
            self.win = [self.w_temp] * ndims #사용함  (11, 11, 11)

        weight_win_size = self.w_temp # 11 
        weight = torch.ones((1, 1, weight_win_size, weight_win_size, weight_win_size), device=I.device, requires_grad=False)    # 1,1,11,11,11
        conv_fn = F.conv3d

        # compute CC squares
        I2 = I*I # moving * moving
        J2 = J*J # fixed * fixed
        IJ = I*J # moving * fixed
