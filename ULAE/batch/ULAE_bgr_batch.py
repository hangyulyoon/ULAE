...
class Uncoupled_Encoding_Block(nn.Module):
    ...
    def forward(self, x, y):
        ...
        fea_e0 = self.mean(x) #nn.AdaptiveAvgPool3d((1, 1, 1))
...
class Accumulative_Enhancement_1(nn.Module):
    ...
    def forward(self, x, y):
        ...
        d2 = self.dc4(d1)
        d3 = self.dc5(d2)
        d6 = self.dc6(torch.cat((self.up(d3), down_down_x_in), 1)) #여기서부터 flow에가까운이미지가생성
...
class ULAE_bgr_batch(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, basic_channel=16, is_train=True, imgshape=(256, 256, 256)):
        super(ULAE_bgr_batch, self).__init__()
    ...
    def forward(self, x, y,input_image,input_image_two):
        ...
        y_source_new_two_b = self.transformer(input_image_two[...,0], flowc)
        ...
        y_source_new_two_end = torch.cat((y_source_new_two_b,y_source_new_two_g), axis=5)
        input_image_warp_two = torch.cat((y_source_new_two_end,y_source_new_two_r), axis=5)



        # y_source_new_two_b_2 = self.transformer(input_image_two[...,0], flow3)
        # y_source_new_two_g_2 = self.transformer(input_image_two[...,1], flow3)
        # y_source_new_two_r_2= self.transformer(input_image_two[...,2], flow3)
        # y_source_new_two_b_2 = y_source_new_two_b_2[...,np.newaxis].cpu().detach().numpy()
        # y_source_new_two_g_2 = y_source_new_two_g_2[...,np.newaxis].cpu().detach().numpy()
        # y_source_new_two_r_2 = y_source_new_two_r_2[...,np.newaxis].cpu().detach().numpy()
        # y_source_new_two_end_2 = np.concatenate((y_source_new_two_b_2,y_source_new_two_g_2), axis=5)
        # input_image_warp_two_2 = np.concatenate((y_source_new_two_end_2,y_source_new_two_r_2), axis=5)
        ...
        return [warp1, warp2, warp3], [flowa, flowb, flowc], [flow1, flow2, flow3]#input_image_warp_two_2

if __name__=="__main__":
    model = ULAE_bgr_batch(2, 3, 16, True, [256,256,256]).cuda() # 160,192,160
