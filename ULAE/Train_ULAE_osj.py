...
def moving_data(moving,fixed,moving_label) :
    ...
    for i in range(maskimg.shape[2]):
        patient_cal_mask=np.zeros((600, 600))
        patient_cal_mask[:,299:301]=1
        cutimg = maskimg[:, :, i] # slice 
        mm = np.zeros((600, 600))
        ...
        for cont in contours:
            ...
            if np.sum(patient_cal_mask.copy()*imgcopy)>0:
                img=img+imgcopy
    ...
    for i in range(dataarray.shape[-1]):    
        for i_2 in range(37):
            if i_2 ==0 :
                moving_zeros[:,:,i] = clahe.apply(np.array(dataarray[:,:,i],dtype='uint8'))
            elif i_2>=1:
                moving_zeros[:,:,i] = clahe.apply(np.array(moving_zeros[:,:,i],dtype='uint8'))
              
        moving_zeros[:,:,i][moving_zeros[:,:,i]<35]=0 # background 0 
        moving_zeros[:,:,i]=moving_zeros[:,:,i]/255 # intensity 0~1 
    ...
    for i in range(dataarray.shape[-1]):    
        for i_2 in range(37):
            if i_2 ==0 :
                fixed_zeros[:,:,i] = clahe.apply(np.array(dataarray[:,:,i],dtype='uint8'))
            elif i_2>=1:
                fixed_zeros[:,:,i] = clahe.apply(np.array(fixed_zeros[:,:,i],dtype='uint8'))

        fixed_zeros[:,:,i][fixed_zeros[:,:,i]<35]=0 #background 0
        fixed_zeros[:,:,i]=fixed_zeros[:,:,i]/255 # intensity 0~1
        print(i)
    ...
    for i in range(moving_label.shape[2]):
        moving_label_s = moving_label[:,:,i] # slice  
    ...
    for i_2 in range(moving_clahe.shape[2]-1,0,-1):
        ...
        if np.sum(moving_crop_2) !=0:
            break
    for i_3 in range(moving_clahe.shape[1]):
        moving_crop_3 = moving_clahe[:,i_3,:]
        if np.sum(moving_crop_3) !=0:
            break  
    for i_4 in range(moving_clahe.shape[1]-1,0,-1):
        moving_crop_4 = moving_clahe[:,i_4,:]
        if np.sum(moving_crop_4) !=0:
            break
    for i_5 in range(moving_clahe.shape[0]):
        moving_crop_5 = moving_clahe[i_5,:,:]
        if np.sum(moving_crop_5) !=0:
            break
    for i_6 in range(moving_clahe.shape[0]-1,0,-1):
        moving_crop_6 = moving_clahe[i_6,:,:]
        if np.sum(moving_crop_6) !=0:
            break
    moving_clahe_crop = moving_clahe[i_5:i_6, i_3:i_4, i:i_2] #(255, 189, 255)
    moving_label_color_crop = moving_label_color[i_5:i_6, i_3:i_4, i:i_2] #(255, 189, 255, 3)
    ...
    for k_6 in range(fixed_clahe.shape[2]-1,0,-1):
        ...
        if np.sum(moving_crop_6) !=0:
            break
    fixed_clahe_crop = fixed_clahe[k_5:k_6, k_3:k_4, k:k_2] # (252, 191, 255)
    
    #ㅡㅡㅡㅡㅡfixed_clahe crop(3d) endㅡㅡㅡㅡㅡ#    
        

    #ㅡㅡㅡㅡㅡmoving_fixed resize startㅡㅡㅡㅡㅡ#
    moving_crop_reisze =resize(moving_clahe_crop,(fixed_clahe_crop.shape[0],fixed_clahe_crop.shape[1],fixed_clahe_crop.shape[2])) # (252, 191, 255)
    moving_label_color_resize =resize(moving_label_color_crop,(fixed_clahe_crop.shape[0],fixed_clahe_crop.shape[1],fixed_clahe_crop.shape[2])) # (252, 191, 255)
    #ㅡㅡㅡㅡㅡmoving_fixed resize endㅡㅡㅡㅡㅡ#

    #ㅡㅡㅡㅡㅡmoving_fixed padding startㅡㅡㅡㅡㅡ#
    moving_crop_reisze_padding = np.pad(moving_crop_reisze,((k_5,fixed_clahe.shape[0]-k_6),(k_3,fixed_clahe.shape[1]-k_4),(k,fixed_clahe.shape[2]-k_2))) # (256, 256, 256)
    moving_label_color_resize_padding = np.pad(moving_label_color_resize,((k_5,fixed_clahe.shape[0]-k_6),(k_3,fixed_clahe.shape[1]-k_4),(k,fixed_clahe.shape[2]-k_2),(0,0))) # (256, 256, 256, 3)
    #ㅡㅡㅡㅡㅡmoving_fixed padding endㅡㅡㅡㅡㅡ#

    return moving_crop_reisze_padding, fixed_clahe, moving_label_color_resize_padding
...
moving_crop_reisze_padding,fixed_clahe,moving_label_color_resize_padding=moving_data(moving_image,fixed_image,moving_label_base)
 

#ㅡㅡㅡㅡㅡ data save start ㅡㅡㅡㅡㅡ#

AA  =nib.Nifti1Image(moving_crop_reisze_padding,None)
nib.save(AA,'/data/tu_data/img_processing/moving_crop_reisze_padding_nobed.nii.gz')
AA  =nib.Nifti1Image(fixed_clahe,None)
nib.save(AA,'/data/tu_data/img_processing/fixed_clahe_nobed.nii.gz')
AA  =nib.Nifti1Image(moving_label_color_resize_padding,None)
nib.save(AA,'/data/tu_data/img_processing/moving_label_color_resize_padding_nobed.nii.gz')

#ㅡㅡㅡㅡㅡ data save end  ㅡㅡㅡㅡㅡ#


moving_crop_reisze_padding= moving_crop_reisze_padding[np.newaxis,np.newaxis,...] # batch, channel, (shape)

fixed_clahe= fixed_clahe[np.newaxis,np.newaxis,...]

moving_label_base = moving_label_base[np.newaxis,np.newaxis,...] 
moving_label_base = torch.from_numpy(moving_label_base).cuda().float()

moving_label_color_resize_padding = moving_label_color_resize_padding[np.newaxis,np.newaxis,...] 
moving_label_color_resize_padding = torch.from_numpy(moving_label_color_resize_padding).cuda().float()
...
imgshape = moving_crop_reisze_padding.shape[2:]
...
def train_ULAE():
    ...
    while step <= iteration:
            ...
            X = torch.from_numpy(moving_crop_reisze_padding).cuda().float()
            Y = torch.from_numpy(fixed_clahe).cuda().float()

            warps, flows, _,input_image_warp,input_image_warp_two = model(X, Y,moving_label_base,moving_label_color_resize_padding)  #return [warp1, warp2, warp3], [flowa, flowb, flowc], [flow1, flow2, flow3]
            ...
            if (step % 10 == 0):
                ...
                nib.save(O,'/data/tu_data/result_nobed/'+str(step)+'result.nii.gz')            
                O=nib.Nifti1Image(input_image_warp[0,0,:,:,:].cpu().detach().numpy(),None)
                nib.save(O,'/data/tu_data/result_nobed/'+str(step)+'result_new.nii.gz')
                O=nib.Nifti1Image(input_image_warp_two[0,0,:,:,:,:].cpu().detach().numpy(),None)
                nib.save(O,'/data/tu_data/result_nobed/'+str(step)+'result_new_two.nii.gz')
