...
def moving_data(moving,fixed,moving_label) :
    ...
    for k_2 in range(fixed_clahe.shape[2]-1,0,-1):
        ...
        if np.sum(moving_crop_2) !=0:
            break
    for k_3 in range(fixed_clahe.shape[1]):
        moving_crop_3 = fixed_clahe[:,k_3,:]
        if np.sum(moving_crop_3) !=0:
            break  
    for k_4 in range(fixed_clahe.shape[1]-1,0,-1):
        moving_crop_4 = fixed_clahe[:,k_4,:]
        if np.sum(moving_crop_4) !=0:
            break
    for k_5 in range(fixed_clahe.shape[0]):
        moving_crop_5 = fixed_clahe[k_5,:,:]
        if np.sum(moving_crop_5) !=0:
            break
    for k_6 in range(fixed_clahe.shape[0]-1,0,-1):
    ...
    moving_label_color_resize_padding = np.pad(moving_label_color_resize,((k_5,256-k_6),(k_3,256-k_4),(k,256-k_2),(0,0))) # (256, 256, 256, 3)
...
# nib.save(AA,'/data/tu_data/img_processing/moving_crop_reisze_padding.nii.gz')
# AA  =nib.Nifti1Image(fixed_clahe,None)
# nib.save(AA,'/data/tu_data/img_processing/fixed_clahe.nii.gz')
# AA  =nib.Nifti1Image(moving_label_color_resize_padding,None)
# nib.save(AA,'/data/tu_data/img_processing/moving_label_color_resize_padding.nii.gz')
